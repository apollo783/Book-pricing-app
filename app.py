...
# Custom CSS to force the uploaded image container to fill the screen width
st.markdown("""
    <style>
        .stImage > img {
            width: 100% !important;
...
tab1, tab2 = st.tabs(["📷 Snap Cover / Barcode", "⌨️ Manual Search"])

book_title = ""

def process_and_compress(file_bytes):
    """Fixes image orientation and compresses to under 300KB while keeping text crisp."""
    img = Image.open(file_bytes)
    img = ImageOps.exif_transpose(img)  # Fix mobile camera orientation
    img.thumbnail((1200, 1200))          # High res enough for crisp OCR
    return img

with tab1:
    st.subheader("Scan Book Cover or Barcode")
    uploaded_file = st.file_uploader(
        "Tap below to take a photo using your phone camera", 
        type=["jpg", "jpeg", "png"]
    )
    
    if uploaded_file is not None:
        img = process_and_compress(uploaded_file)
        
        # Displays photo prominently filling over 50% of the screen height
        st.image(img, caption="Captured Cover", use_container_width=True)
        ...
        if barcodes:
            ...
            try:
                ...
                if clean_lines:
                    ...
                    book_title = st.text_input("Confirm Title:", value=detected_text)
                else:
                    st.warning("Could not read text clearly. Type title below:")
                    book_title = st.text_input("Enter Book Title:")
            except Exception:
                st.warning("OCR failed. Type title below:")
                book_title = st.text_input("Enter Book Title:")
...
if book_title:
    ...
    st.divider()
    st.subheader("🏷️ Pricing Calculator")
